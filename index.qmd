...
groups_summary <- literature_merged_data |>
  group_by(
    ENVIRON_COMPARTMENT,
  ...
  # ignore wet weight, there's less of it and scaling is complicated
  filter(
    MEASURED_UNIT_STANDARD %in% c("mg/L", "mg/kg (dry)"),
  ...
  reframe(
    n = sum(MEASURED_N),
    # we can't actually just take the mean, there's lots of other stuff to consider
...
# TODO Exclude irreg. units
# TODO: Calculate distributions properly (with uncertainty)
# Then pull mode, mean, whatever
# Get number of outliers, do hartigan's dip test
# Decide what to inspect based on that

# for the sake of argument, flag any group with over 5% outliers
groups_summary_flagged <- groups_summary |>
  filter((n_double_outliers / n) > 0.05)

groups_summary_flagged |> ggplot
```
...
Data were grouped by combinations of *Environmental compartment*, *environmental subcompartment*, location, Site geographical feature, date, study, and in the case of biotic samples, Species and tissue, and distributions plotted, in order to visually review data. 

Ultimately, our goal was to find statistically reasonable, plausible groups of kinetically,  environmentally, and geographically similar/contiguous matrices that could be used as KES in our AEPs.
