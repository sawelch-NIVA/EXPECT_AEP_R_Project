...
#| tbl-caption: Summary statistics of datapoints grouped (sub-)compartment, species group, species, tissue, and geographical (sub-)feature. Groups where <= 100 have been excluded for reasons of space (81 groups, 1723 samples); please see supplementary information for full table. Outliers refers to proportion of data points flagged by both Tukey's Fences *both* Tukey Fences ($Z >= 1.5$) and Robust-Modified Z-Score ($Z >= 3.5$); rows highlighted yellow when >=5% double-flagged. multimodal column gives p-value where Hartigans' dip test found significant non-unimodality (red text), empty row indicates non-significance, significantly unimodal, or insufficient n. At this point in the analysis, wet and dry weight means are calculated separately.

summarised_literature_data <- tar_read(summarise_literature_data)

# Flag groups with >5% outliers or non-unimodal distributions
groups_summary_flagged <- summarised_literature_data |>
  filter((n_double_outliers / n) > 0.05 | multimodal)

summarised_literature_data_n_gt_100 <- summarised_literature_data |>
  filter(n >= 100)

# Build display table; stash flag columns so indices stay correct after arrange()
tbl_data <- summarised_literature_data_n_gt_100 |>
  mutate(
    # Fold compartment + taxonomy into one column
    Group = if_else(
      ENVIRON_COMPARTMENT == "Biota",
      paste(
        coalesce(SPECIES_GROUP, "Unknown"),
        coalesce(SAMPLE_SPECIES, "spp."),
        coalesce(SAMPLE_TISSUE, "—"),
        sep = " › "
      ),
      paste(ENVIRON_COMPARTMENT, ENVIRON_COMPARTMENT_SUB, sep = " › ")
    ),
    ...
    Dates = paste0(format(date_min, "%Y"), "–", format(date_max, "%Y")),
    N = as.integer(n),
    # Fold mean, SD, and unit into one cell
    `Mean ± SD` = sprintf("%.2g ± %.2g %s", mean, sd, unit),
    Median = median,
    Outliers = n_double_outliers,
    `multimodal (p)` = if_else(
      multimodal,
      formatC(dip_p, digits = 2, format = "f"),
      ""
    ),
    # Temp flags — recalculated after arrange() below
    .is_multimodal = multimodal,
    .is_outlier = (n_double_outliers / n) > 0.05,
    .keep = "none"
  ) |>
  arrange(Group, Location)

multimodal_idx <- which(tbl_data$.is_multimodal)
outlier_idx <- which(tbl_data$.is_outlier)

tbl_data |>
  select(-.is_multimodal, -.is_outlier) |>
  flextable() |>
  theme_vanilla() |>
  bold(part = "header") |>
  colformat_double(j = "Median", digits = 2) |>
  # Merge repeated values in grouping columns to reduce visual height
  merge_v(j = c("Group", "Location", "Dates")) |>
  color(i = multimodal_idx, j = "Multimodal (p)", color = "red") |>
  bg(i = outlier_idx, bg = "#FFF3CD") |>
