...
-   [x] Why are upper and lower uncertainty values flipped?
...
print("Not working. Quarto makes me sad.")
```
...
@fig-boxplot-aquatic-liquid shows we have a fair amount of data for water. It's not strictly useful to include saltwater, freshwater and inbetween on here, so I think it makes sense to split it - but it won't make a massive difference to the final product.

```{r}
#| label: fig-boxplot-aquatic-liquid-fresh
#| fig-cap: "Copper concentrations in water column samples by reference"
#| fig-width: 10

# # why are 3 rows out of scale range
# right, cos of LOD/LOQ
temp <- literature_merged_data |> group_by(MEASURED_OR_IMPUTED_VALUE_STANDARD) |> reframe(count = n()) |> arrange(desc(count))
