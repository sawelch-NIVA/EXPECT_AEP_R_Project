...
# vm data isn't in targets yet, but I want to pull it in and see what happens
# this is all terrible code
vm_measurements_2025 <- fread("data/clean/Vm_Unzipped/measurements.csv")
vm_campaign_2025 <- fread("data/clean/Vm_Unzipped/campaign.csv") |> select(-ENTERED_BY, -ENTERED_DATE)
vm_sites_2025 <- fread("data/clean/Vm_Unzipped/sites.csv") |> select(-ENTERED_BY, -ENTERED_DATE)
vm_reference_2025 <- fread("data/clean/Vm_Unzipped/reference.csv")

vm_merged_data <- vm_measurements_2025 |> left_join(vm_campaign_2025, by = "CAMPAIGN_NAME_SHORT")
vm_merged_data <- vm_merged_data |> left_join(vm_sites_2025, "SITE_CODE")
vm_merged_data <- vm_merged_data |> left_join(vm_reference_2025, "REFERENCE_ID") |>  mutate(SUBSAMPLE = as.character(SUBSAMPLE),
SAMPLING_PROTOCOL = as.character(SAMPLING_PROTOCOL),
ANALYTICAL_PROTOCOL = as.character(ANALYTICAL_PROTOCOL),
FRACTIONATION_PROTOCOL = as.character(FRACTIONATION_PROTOCOL),
EXTRACTION_PROTOCOL = as.character(EXTRACTION_PROTOCOL))

# more date format woes. fread is now reading as dttm instead of IDATE?
vm_merged_data[, SAMPLING_DATE := as.IDate(SAMPLING_DATE)]

# Fuck it. Ok, this worked. So it was another date and I was assuming SAMPLING_DATE. Hmm.
vm_merged_data <- vm_merged_data |>
  mutate(across(
    contains("DATE"),
    ~ as.IDate(.x)
  ))
# for fuck's sake
sapply(literature_merged_data, class)
sapply(vm_merged_data, class)

# I give up.
literature_merged_data <- literature_merged_data |> bind_rows(vm_merged_data)
# Error in `as.Date.default()`:
# ! do not know how to convert 'e' to class “Date”
# This is completely uninformative..
