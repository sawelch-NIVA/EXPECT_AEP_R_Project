...
message("Joined data to lookup tables")
```
...
vm_filtered <- vm_merged |>
  ...
  filter(
    ENVIRON_COMPARTMENT_vkat == config$filters$compartment,
    ENVIRON_COMPARTMENT_medium == config$filters$compartment,
    ENVIRON_COMPARTMENT_SUB_vkat == config$filters$compartment_sub,
    ENVIRON_COMPARTMENT_SUB_medium == config$filters$compartment_sub
  ) |>
...
A sankey diagram would be better, but this will do for now.
