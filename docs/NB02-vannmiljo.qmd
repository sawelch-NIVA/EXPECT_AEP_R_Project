...
vm_summary_methods_table <- vm_summary_methods |>
...
resolve_compartment_conflicts <- function(df) {
  df <- df |>
    mutate(
      ...
      ENVIRON_COMPARTMENT_resolved = case_when(
        ...
        # if there's no conflict, use the first value
        ENVIRON_COMPARTMENT_vkat ==
          ENVIRON_COMPARTMENT_medium ~ ENVIRON_COMPARTMENT_medium,
      ...
      ENVIRON_COMPARTMENT_SUB_resolved = case_when(
        ...
        # if ENVIRON_COMPARTMENT_SUB_vkat is wildcard then use ENVIRON_COMPARTMENT_SUB_medium
        ...
        # if ENVIRON_COMPARTMENT_SUB_medium is wildcard then use ENVIRON_COMPARTMENT_MEDIUM_vkat
        !has_data_issue(ENVIRON_COMPARTMENT_SUB_vkat) &
          !has_data_issue(ENVIRON_COMPARTMENT_SUB_medium) &
          ENVIRON_COMPARTMENT_SUB_vkat != "*" &
  ...
  if (n_unresolved + n_unresolved_sub > 0) {
    warning("Some (sub)compartments still aren't resolved.")
  }
...
vm_compartment_conflicts_resolved_filter <- vm_compartment_conflicts_resolved |>
  filter(
    ENVIRON_COMPARTMENT_SUB_resolved != "FLAG: Compartment conflict." &
      ENVIRON_COMPARTMENT_resolved != "FLAG: Compartment conflict."
  )

if (nrow(unresolved_conflicts_summary) > 0) {
  warning(glue(
    "{nrow(unresolved_conflicts_summary)} compartment conflict combinations remain unresolved ({sum(unresolved_conflicts_summary$count)} rows) and have been removed from the dataset."
  ))
}
...
vm_geo_conflicts_flagged <- vm_compartment_conflicts_resolved |>
  mutate(
...
# Filter out geographic conflicts
# FIXME: For some reason this also drops all rows where ENVIRON_COMPARTMENT_resolved == "Biota"
vm_compartment_geo_conflicts_removed <- resolve_geographic_conflicts(
  vm_geo_conflicts_flagged
) |>
  filter(
...
message(glue(
  "Removed {nrow(vm_geo_conflicts_flagged) - nrow(vm_compartment_geo_conflicts_removed)} rows with unresolved geographic conflicts"
))

# I really don't understand what's going on here...
vm_compartment_geo_conflicts_for_table <- resolve_geographic_conflicts(
  vm_geo_conflicts_flagged
) |>
  filter(
    SITE_GEOGRAPHIC_FEATURE_resolved == "FLAG: Geographic conflict." |
...
vm_compartment_sites_split <- vm_compartment_geo_conflicts_removed |>
...
conflict_flow <- filter_flow |>
  add_row(
    stage = "Compartment Resolution",
    records = nrow(vm_compartment_conflicts_resolved),
    pct = round(100 * records / first(filter_flow$records), 1),
    label = glue("{scales::comma(records)}\n({pct}%)")
  ) |>
  add_row(
    stage = "Geographic Conflicts Removed",
    records = nrow(vm_compartment_sites_split),
    pct = round(100 * records / first(filter_flow$records), 1),
    label = glue("{scales::comma(records)}\n({pct}%)")
  )

# Simple bar plot
ggplot(conflict_flow, aes(x = stage, y = records)) +
  geom_col(aes(fill = stage)) +
  geom_text(aes(label = label), vjust = -0.5, size = 3) +
  scale_fill_brewer(palette = "Set1") +
  scale_x_discrete(limits = conflict_flow$stage) +
  scale_y_continuous(
    labels = scales::comma,
    expand = expansion(mult = c(0, 0.1))
  ) +
