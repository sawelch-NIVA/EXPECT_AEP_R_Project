...
resolve_compartment_conflicts <- function(df) {
  df <- df |>
    mutate(
      ENVIRON_COMPARTMENT_SUB_resolved = case_when(
        ...
        # Add more rules as you discover conflicts
        # Rule 2: ...
        # Rule 3: ...
  ...
  n_resolved <- df |> tally(!is.na(ENVIRON_COMPARTMENT_SUB_resolved))
  message(glue("Resolved {n_resolved} conflicting rows."))

  df
}

vm_compartment_conflicts_resolved <- resolve_compartment_conflicts(vm_filtered)
```
...
edata_measurements <- vm_compartment_conflicts_resolved |>
  mutate(
    # Core identifiers
    SITE_CODE = glue("Vannmiljø_{Vannlok_kode}"),
    PARAMETER_NAME = "Copper",
