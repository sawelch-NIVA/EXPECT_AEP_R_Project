...
flowchart LR
  A[Export samples from Vannmijlø] --> C(Knitr)
  B[Export sites from Vannmijlø] --> C(Knitr)
  C(Knitr)
```
...
Let's update the not-Sankey now that we've removed some more rows.

```{r}
#| label: fig-conflict_resoltion-visualisation
...
conflict_flow <- filter_flow |>
  add_row(
    ...
    records = nrow(vm_compartment_conflicts_resolved_filter),
...
We also need to convert tissue types (stored as MediumID in Vannmiljø) into our appropriate equivalent.
