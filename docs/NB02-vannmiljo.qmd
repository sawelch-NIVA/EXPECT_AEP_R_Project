...
config <- list(
  ...
  filters = list(
    ...
    compartment = "Aquatic",
    compartment_sub = c("Freshwater", "Aquatic Sediment")
  ),
...
vm_lookup_campaigns <- read_csv(config$files$lookup_campaigns)
...
message(
  "Joined data to lookup tables: vm_lookup_medium, vm_lookup_vannkategori, vm_lookup_campaigns"
)
```
...
```{r}
#| label: fig-heatmap-campaign-medium
...
```{r}
#| label: fig-heatmap-campaign-species
  ...
  df |>
    mutate(
      ENVIRON_COMPARTMENT_SUB_resolved = case_when(
        ...
        # Default: prefer one source consistently
