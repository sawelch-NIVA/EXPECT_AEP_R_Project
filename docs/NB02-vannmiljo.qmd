...
Finally, what about units @fig-heatmap-units-medium? Units are, I'm releieved to say, sensible.
...
filter_flow <- tibble(
  stage = c("Raw", "Compartments", "Point sites", "No biota", "Date range"),
  records = c(
    nrow(vm_merged),
    vm_merged |>
      filter(
...
# then merge everything to ENVIRON_COMPARTMENT_SUB. this is what we'll use going forwards
vm_compartment_conflicts_resolved <- resolve_compartment_conflicts(
  vm_filtered
) |>
  mutate(
    ENVIRON_COMPARTMENT_SUB = case_match(
      ENVIRON_COMPARTMENT_SUB_resolved,
      NA_character_ ~ ENVIRON_COMPARTMENT_SUB_resolved,
      .default = ENVIRON_COMPARTMENT_SUB_medium
    ),
    ENVIRON_COMPARTMENT = case_match(
      ENVIRON_COMPARTMENT_resolved,
      NA_character_ ~ ENVIRON_COMPARTMENT_resolved,
...
if (any(geo_check$geo_conflict | geo_check$geo_sub_conflict, na.rm = TRUE)) {
  warning(glue(
...
edata_samples <- vm_compartment_conflicts_resolved |>
  mutate(
    # Core identifiers
    SITE_CODE = glue("Vannmiljø_{Vannlok_kode}"),
    SITE_NAME = glue("Vannmiljø Station {Vannlokalitetsnavn}"),
...
biota_samples_merged <- edata_samples |> bind_rows(vm_samples_biota_only)
...
edata_measurements <- biota_samples_merged |>
  mutate(
    ...
    LOQ_UNIT = "µg/L", # TODO: Fix LOQ units
    LOD_VALUE = Deteksjonsgrense,
    LOD_UNIT = "µg/L", # TODO: Fix LOD units
...
count(edata_measurements, SAMPLE_SPECIES)


edata_measurements |>
  ggplot(aes(x = MEASURED_VALUE, y = ENVIRON_COMPARTMENT)) +
  geom_point()
```
