...
plot_data <- literature_merged_data |>
  ...
  distinct() |>
  reframe(count = sum(MEASURED_N, na.rm = TRUE), SPECIES_GROUP, SPECIES_COMMON_NAME) |>
  # Join year data for y-axis ordering
...
hb <- hexbin(sites_sample_number$LONGITUDE, 
...
sites_sample_number_proj <- sites_sample_number |> filter(!is.na(LONGITUDE), !is.na(LATITUDE)) |> 
...
world_map_proj <- sf::st_as_sf(world_map, 
                                coords = c("long", "lat"), 
                                crs = 4326) |>
...
ggplot() +
  ...
  #   size = 3
  # ) +
  scale_fill_viridis_b(name = "Samples", breaks = c(1, 5, 10, 50, 100, 500, 1000, 2000)) +
  scale_x_continuous(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0)) +
  theme_minimal() +
  theme(axis.text = element_blank(), axis.title = element_blank()) +
  coord_fixed(ratio = 1,     xlim = c(
