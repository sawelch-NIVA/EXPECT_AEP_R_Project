...
test_that("here_rel() returns a relative path", {
  expect_false(fs::is_absolute_path(here_rel("data/clean/decisions/group_decisions.csv")))
...
test_that("here::here() is absolute, which is the whole reason for this function", {
  ...
  expect_true(fs::is_absolute_path(here::here("data/clean/decisions/group_decisions.csv")))
})
...
test_that("here_rel() and here::here() resolve to the same location", {
  rel <- here_rel("data/clean/decisions/group_decisions.csv")
  abs <- here::here("data/clean/decisions/group_decisions.csv")
  expect_identical(
...
test_that("here_rel() is relative to the working directory, not the root", {
  ...
  withr::with_dir(here::here("R"), {
    out <- here_rel("data/clean/decisions/group_decisions.csv")
    expect_false(fs::is_absolute_path(out))
    expect_match(out, "^\\.\\./")
    expect_identical(
      normalizePath(out, winslash = "/", mustWork = FALSE),
      normalizePath(
        here::here("data/clean/decisions/group_decisions.csv"),
        winslash = "/",
        mustWork = FALSE
      )
    )
