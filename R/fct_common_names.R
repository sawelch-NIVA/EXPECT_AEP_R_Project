...
get_common_names <- function(
  ...
  input_sym <- rlang::sym(input_col)
  output_sym <- rlang::sym(output_col)

  # Load existing cache if it exists
  if (file.exists(cache_path)) {
    cached_names <- readr::read_csv(cache_path, show_col_types = FALSE)
    if (verbose) message("Loaded cache with ", nrow(cached_names), " species")
  } else {
    cached_names <- tibble::tibble(
  ...
  species_to_lookup <- biota_data |>
    dplyr::pull(!!input_sym) |>
    unique()

  # Filter out species already in cache for this specific db
  cached_for_db <- cached_names |> dplyr::filter(db == !!db)
  ...
  if (length(new_species) > 0) {
    ...
    common_names_list <- taxize::sci2comm(new_species, db = db, simplify = TRUE)
    ...
    updated_cache <- dplyr::bind_rows(cached_names, new_results)

    # Save updated cache
    readr::write_csv(updated_cache, cache_path)
